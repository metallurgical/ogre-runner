...
- Pick one of two plain waiting strategies, based on whether ogre's own `--background` flag was passed:
  - **No ogre `--background` flag** (default `new`/foreground isolation): wrap the blocking call in a plain backgrounded shell command (e.g. Bash tool's `run_in_background: true`) — always, not just for `--all` chains. Even a single-step `ogre execute`/`ogre feature`/`ogre review-plan`/`ogre add-blocker`/`ogre rescue` call blocks the underlying shell until its isolated subprocess finishes, so leaving it as a plain synchronous call still hard-blocks the conversation for that duration and never surfaces as a trackable/clickable background job in the harness's own job list. Wrapping it costs nothing extra and gets both: non-blocking + a single completion signal the moment it exits, no polling either way. Use this by default for every non-`--main` invocation of these subcommands, regardless of step count.
...
- **Every codex spawn runs fully unsandboxed** (`--dangerously-bypass-approvals-and-sandbox`, unconditional as of the retirement of `--codex-unsandboxed-browser-check`/`codex_unsandboxed_browser_check` - that opt-in flag/config no longer exists). Codex's default sandbox otherwise blocks things like real registry access and browser subprocess spawns outright; `claude` isolates fine by default and doesn't need this. Ogre is a dev-only tool as a result - never point it at a production environment or anything you wouldn't hand full filesystem/shell/network access to. If the user hasn't already acknowledged this tradeoff (e.g. first `ogre execute --executor codex` in a session), say so before running it.

...
* [ ] Step two.
* [ ] Step three.
* [ ] `[BROWSER-CHECK]` Only ever tag ONE step this way, and only when `--browser-check` is active AND this plan makes any UI/frontend change (new/modified view, page, component, template, or user-facing route). Append it as the FINAL step, titled "End-to-end acceptance walkthrough", after every other implementation step (last or second-to-last, before final lint/build/test). It exercises every item in §7 Acceptance Criteria live in the browser - nav entry point, the primary happy path, each documented filter/state variant named in the plan, and the empty/edge state - not per-component rendering checks. Do NOT tag any other step `[BROWSER-CHECK]`, even ones that create or modify a view - per-step live-render checks are retired in favor of this single end-to-end pass. If the plan is backend-only (no UI/view/template change), omit this step entirely - `[BROWSER-CHECK]` does not apply. `ogre execute` runs a `[BROWSER-CHECK]` step in an isolated spawn like any other step, expecting the executor to verify it with a browser MCP (e.g. Playwright); if no browser MCP is detected for the executor it auto-falls back to `--main` so the step still completes in the current session. Any bug found during this step is fixed inline by the executor (an ad-hoc auto-fix task, not a new numbered plan step) before the step is marked passed.

...
- `--mcp-config PATH` — browser MCP config handed to the spawned `claude` session so `[BROWSER-CHECK]` steps run isolated instead of falling back to `--main`. Also settable persistently as `"browser_mcp"` in `.ai/.ogre/config.json`. (codex uses its own `~/.codex/config.toml` `mcp_servers` instead.)
...
- **Browser MCP available** (a Playwright/browser MCP is configured for the executor — ambient project/user MCP, `browser_mcp` in `.ai/.ogre/config.json`, or `--mcp-config PATH`): the spawned session verifies the step with its own browser tools. Nothing special for you to do — single steps and `--all` chains run straight through, main context untouched. This is the goal state; prefer configuring a browser MCP so browser-check never touches this session.

...
- **No browser MCP detected**: Ogre falls back so the step still completes automatically (no manual retrigger):
  ...
  3. **`--all --background`**: first decide, at launch time, whether this run is **isolated-capable** for `[BROWSER-CHECK]` — `claude` executor with a browser MCP actually configured (ambient `claude mcp list`, `browser_mcp` in config, or `--mcp-config`), or `codex` executor with **both** `--codex-unsandboxed-browser-check`/config **and** a confirmed playwright/puppeteer entry in `codex mcp list` (check it yourself — don't assume from the flag alone). This is a property of the whole run, decided once, not something to re-derive per step.

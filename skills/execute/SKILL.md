...
this file's own flag list above (or `scripts/ogre`'s actual parsing) before running
anything — never guess and substitute. Only translate into a flag yourself when the
user names no flag at all and describes pure intent in plain English (e.g. "resume
where it left off" → you choose `--resume`, having actually checked what it does
first).
...
- **No browser MCP detected**: Ogre falls back so the step still completes automatically (no manual retrigger):
  ...
  3. **`--all --background`**: **regardless of anything below, `ogre execute --background` never notifies this session on its own when the chain finishes** — it self-detaches into its own process group, so nothing calls back here. The moment you launch it, start a poll loop yourself in this same session (one Bash tool call, `run_in_background: true`, wrapping `while :; do ${CLAUDE_PLUGIN_ROOT}/scripts/ogre status <issue>; sleep 20; done` breaking on `completed`/`stopped`/`failed`/`blocked`) — do this even when the plan has zero `[BROWSER-CHECK]` steps. "No browser-check steps" only means nothing below will make the poll loop stop early to intervene; it is not a reason to skip starting the loop in the first place. A prior version of this doc said "None tagged → just kick off `--background` and stop; nothing will pause" here, which a live session read as "no polling needed" and silently left a finished chain unreported for hours — don't repeat that.
     Separately, decide at launch time whether this run is **isolated-capable** for `[BROWSER-CHECK]` — `claude` executor with a browser MCP actually configured (ambient `claude mcp list`, `browser_mcp` in config, or `--mcp-config`), or `codex` executor with a confirmed playwright/puppeteer entry in `codex mcp list` (check it yourself — codex is always unsandboxed now, so the MCP presence alone is the only signal, no flag to check). This only decides whether the poll loop ever needs to intervene mid-chain, not whether the loop exists at all.
     ...
     - **Not isolated-capable** (no MCP found): grep the plan's remaining unchecked items for `[BROWSER-CHECK]`. None tagged → the poll loop above just waits for `completed`/`stopped`/`failed`/`blocked`, nothing else to do. At least one tagged → whenever `current_step` contains `[BROWSER-CHECK]` and the job isn't `completed`/`stopped`, resolve it like case 2 (`--main`, real check, `task-complete`, resume with `--all --background` preserving flags), then keep polling the same way.
...
The cleanest fix for all of this is to give the executor a browser MCP once — then every case above collapses to "runs isolated, nothing to intervene on." The completion poll loop from case 3 above still runs regardless — that's for chain-finished notification, not browser-check specifically.
